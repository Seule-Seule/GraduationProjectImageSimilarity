...
double ColorHistogram(Mat &leftImage, Mat &rightImage, string leftSavePath, string rightSavePath)
{
    ...
    if(leftImage.channels() == 1 && rightImage.channels() == 1){
        ...
        int channels3[] = { 0,1 };
        calcHist(&leftSrc, 1, channels3, Mat(), histLeft, 1, histsize, ranges, false, false);
        calcHist(&rightSrc, 1, channels3, Mat(), histRight, 1, histsize, ranges, false, false);
    }
...
void ColorHistTest1()
{
    ...
    std::string resultPath = path + string("/ColorHistTest1");
    if (_access(resultPath.c_str(), 0) == -1)
        _mkdir(resultPath.c_str());

    // 存储结果： 名字1 分辨率1 分辨率2 检测结果
    std::string resultCsvPath = path + string("/ColorHistTest1/result.csv");
    ofstream csvFile;
    //ios::app指从文件尾开始输出
    ...
    for (string file : files)
    {
        ...
        string resultImagePath = path + string("/ColorHistTest1/") + fileName;
        ...
        for (size_t index = 1; index < resolutions.size(); index++){
            ...
            double result = ColorHistogram(image0, image1, imageSavePath0, imageSavePath1);
...
void ColorHistTest2()
{
    ...
    std::string resultPath = path + string("/ColorHistTest2");
    if (_access(resultPath.c_str(), 0) == -1)
        _mkdir(resultPath.c_str());

    // 存储结果： 名字1 分辨率1 分辨率2 检测结果
    std::string resultCsvPath = path + string("/ColorHistTest2/result.csv");
    ofstream csvFile;
    //ios::app指从文件尾开始输出
    csvFile.open(resultCsvPath,ios::out|ios::app);
    if(!csvFile)
    ...
    for (string file : files)
    {
        ...
        string resultImagePath = path + string("/ColorHistTest2/") + fileName;
        if (_access(resultImagePath.c_str(), 0) == -1)
            _mkdir(resultImagePath.c_str());
...
void ColorHistTest3()
{
    ...
    std::string resultPath = path + string("/ColorHistTest3");
    if (_access(resultPath.c_str(), 0) == -1)
        _mkdir(resultPath.c_str());

    // 存储结果： 名字1 名字2 分辨率1 分辨率2 检测结果
    std::string resultCsvPath = path + string("/ColorHistTest3/result.csv");
    ofstream csvFile;
    //ios::app指从文件尾开始输出
    csvFile.open(resultCsvPath,ios::out|ios::app);
    if(!csvFile)
    ...
    for (string file : files)
    {
        ...
        string resultImagePath = path + string("/ColorHistTest3/") + fileName;
        if (_access(resultImagePath.c_str(), 0) == -1)
            _mkdir(resultImagePath.c_str());
...
{
    std::string path("Z:/testImages/images");
    std::string resultPath = path + string("/ColorHistTest4");
    if (_access(resultPath.c_str(), 0) == -1)
        _mkdir(resultPath.c_str());

    // 存储结果： 名字1 名字2 分辨率1 分辨率2 检测结果
    std::string resultCsvPath = path + string("/ColorHistTest4/result.csv");
    ofstream csvFile;
    //ios::app指从文件尾开始输出
    csvFile.open(resultCsvPath,ios::out|ios::app);
    if(!csvFile)
    ...
    for (size_t fileIndex = 0; fileIndex < files.size()-1; fileIndex+=2)
    {
        ...
        string resultImagePath = path + string("/ColorHistTest4/") + fileName0;
        if (_access(resultImagePath.c_str(), 0) == -1)
            _mkdir(resultImagePath.c_str());
        ...
        for (size_t index = 1; index < resolutions1.size(); index++){
            ...
            string resultImagePath = path + string("/ColorHistTest4/") + fileName1;
            if (_access(resultImagePath.c_str(), 0) == -1)
                _mkdir(resultImagePath.c_str());
